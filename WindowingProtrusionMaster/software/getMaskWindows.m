...
[startPoint,nPara,startContour,showPlots,doChecks,spOnly,ssOnly,iFrame,target_pixel_history, smoothedEdge] = parseInput(varargin);

%% 4/11/2020 by Junbong Jang at QCI lab
[orig_mask_rows, orig_mask_cols] = size(maskIn);
[M, N] = size(maskIn);

% Crop mask based on two tracked points in protrusion step.
[maskIn, x_min, y_min, x_max, y_max] = cropBeforeWindowing(target_pixel_history, smoothedEdge, iFrame, maskIn);


%Get distance transform of mask for contouring
distX = bwdist(~maskIn);

%% reexpands the mask image back to the original size
top_expand = y_min - 1;
bottom_expand = orig_mask_rows - y_max;
left_expand = x_min - 1;
right_expand = orig_mask_cols - x_max;

% distX = zeros(M,N);
% distX(y_min:y_max, x_min:x_max) = old_distX;

% distX = padarray(distX, top_expand, 0, 'pre');
distX = padarray(distX, bottom_expand, 0, 'post');
distX = padarray(distX,[0 left_expand], 0, 'pre'); 
% distX = padarray(distX,[0 right_expand], 0, 'post'); 

% divide to reduce the number of parallel contours that are windowed, for faster computation
maxDist = ceil(max(distX(:))/5)
% maxDist = ceil(max(distX(:)))
figure(2);
imshow(distX);
...
% [croppedMaskIn, x_min, y_min, x_max, y_max] = cropBeforeWindowing(target_pixel_history, smoothedEdge, iFrame, maskIn);
% 
% % Remove Contours not in between the tracked points
% for contour_index = 1:size(contours, 1)
%     x_contour_indices_min = contours{contour_index}(1,:) >= x_min;
%     x_contour_indices_max = contours{contour_index}(1,:) <= x_max;
%     y_contour_indices_min = contours{contour_index}(2,:) >= y_min;
%     y_contour_indices_max = contours{contour_index}(2,:) <= y_max;
%     
%     % indices within the cropped box
%     contour_indices = x_contour_indices_min & x_contour_indices_max & y_contour_indices_min & y_contour_indices_max;
%     contours{contour_index} = contours{contour_index}(:,contour_indices);
% end
% 
% % Remove small contour
% min_contour_length = size(contours{1}, 2)/6;
% new_contours = [];
% for contour_index = 1:size(contours, 1)
%     if  size(contours{contour_index}, 2) > min_contour_length
%         new_contours{contour_index,1} = contours{contour_index,1};
%     else
%         disp('contour ignored');
%     end
% end
% contours = new_contours(~cellfun('isempty',new_contours));

% Clean up these contours...
% this function flips row and col in contours
...
% [13.3003479932658,1595.62307035494]
if ~isempty(startPoint)        
...
%         tmp = cell(1,nWinPara(i)+1);
        tmp = cell(1,nWinPara(i));
        X_gradient = contours{iPerpStart(i)}(1,paraVertInd{i}(~sOnBound));
        Y_gradient = contours{iPerpStart(i)}(2,paraVertInd{i}(~sOnBound));
...
%% ------- Slicing Ends ---- %%
% After Slicing, select slices before windowing
% get index of corresponding tracked pixel in the smoothedEdge
% pixel_a = target_pixel_history{iFrame, 1};
% pixel_b = target_pixel_history{iFrame, 2};
% 
% slice_starting_pixels = [];
% for slice_index = 1:size(slices,2)
%     a_slice = slices{1,slice_index};
%     slice_starting_pixels(slice_index, 1) = a_slice(1, 1);
%     slice_starting_pixels(slice_index, 2) = a_slice(2, 1);
% end
% 
% slice_a_index =  findClosestTargetPixel(pixel_a, slice_starting_pixels);
% slice_b_index =  findClosestTargetPixel(pixel_b, slice_starting_pixels);
% 
% slices = slices(1, slice_a_index: slice_b_index);
% paraVert = paraVert(:, slice_a_index: slice_b_index);
% paraVertInd = paraVertInd(1, slice_a_index: slice_b_index);
...
%%
function [startPoint,nPara,startContour,showPlots,doChecks,spOnly,ssOnly,iFrame,target_pixel_history,smoothedEdge] = parseInput(argArray)
...
for i = 1:2:nArg
    ...
    switch argArray{i}
        ...
        case 'target_pixel_history'
            target_pixel_history = argArray{i+1};
            
        case 'smoothedEdge'
            smoothedEdge = argArray{i+1};
