...
% manipulates the fact that pixel_list_last, pixel_list and pixel_t_output have the same indexes
clicked_counts = size(inputParam.target_pixel_list,1);
orig_pixels = inputParam.pixel_list_last;
moved_pixels = pixel_t_output;

for clicked_index = 1:clicked_counts
    orig_pixel_row = findClosestTargetPixel(inputParam.target_pixel_list(clicked_index,:), orig_pixels);
    output.target_pixel_list(clicked_index,:) = moved_pixels(orig_pixel_row, :);
end
% orig_pixel_row_start = findClosestTargetPixel(inputParam.target_pixel_start, orig_pixels);
% orig_pixel_row_end = findClosestTargetPixel(inputParam.target_pixel_end, orig_pixels);
% disp('----------------------');
% output.target_pixel_start = moved_pixels(orig_pixel_row_start, :)
% output.target_pixel_end = moved_pixels(orig_pixel_row_end, :)
%%
if ~inputParam.batch_processing
