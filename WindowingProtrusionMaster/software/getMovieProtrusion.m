...
imgDir = movieData.processes_{1,1}.inFilePaths_(1,1);
img_struct = dir([imgDir{1,1}, '/*.tif']);
img_folder_path = img_struct.folder;
...
target_pixel_history = cell(nFrames,2);
for iFrame = 2:nFrames
    ...
    %% 3/11/2020 added by Junbong for pixel tracking
    
    samIn.target_pixel_list = [];
    if iFrame == 2
        
        img_fullname = [img_folder_path, '\',img_names{1,1}];
        img_data = imread(img_fullname);
        img_data = imadjust(img_data);
        [clicked_x, clicked_y] = clickImage(img_data, 1);
        
        clicked_counts = size(clicked_x,1);
        for clicked_index = 1:clicked_counts
            samIn.target_pixel_list = [samIn.target_pixel_list; clicked_x(clicked_index), clicked_y(clicked_index)];
        end
        %samIn.target_pixel_start = [clicked_x(1), clicked_y(1)];
        %samIn.target_pixel_end = [clicked_x(2), clicked_y(2)];
    else
        for clicked_index = 1:clicked_counts
            samIn.target_pixel_list = [samIn.target_pixel_list; samOut.target_pixel_list(clicked_index,1), samOut.target_pixel_list(clicked_index,2)];
        end
        %samIn.target_pixel_start = samOut.target_pixel_start;
        %samIn.target_pixel_end = samOut.target_pixel_end;
    end
    
    for clicked_index = 1:clicked_counts
        target_pixel_history{iFrame-1,clicked_index} = [samIn.target_pixel_list(clicked_index,1), samIn.target_pixel_list(clicked_index,2)];
    end
    %target_pixel_history(iFrame-1,1) = samIn.target_pixel_start(1);
    %target_pixel_history(iFrame-1,2) = samIn.target_pixel_start(2);
    %target_pixel_history(iFrame-1,3) = samIn.target_pixel_end(1);
    %target_pixel_history(iFrame-1,4) = samIn.target_pixel_end(2);
...
%% 3/11/2020 added by Junbong for pixel tracking
for clicked_index = 1:clicked_counts
    target_pixel_history{iFrame,clicked_index} = [samOut.target_pixel_list(clicked_index,1), samOut.target_pixel_list(clicked_index,2)];
end
% target_pixel_history(nFrames,1) = samOut.target_pixel_start(1);
% target_pixel_history(nFrames,2) = samOut.target_pixel_start(2);
% target_pixel_history(nFrames,3) = samOut.target_pixel_end(1);
% target_pixel_history(nFrames,4) = samOut.target_pixel_end(2);
...
save([p.OutputDirectory filesep fName '.mat'],...
            'protrusion','normals','smoothedEdge','target_pixel_history');
