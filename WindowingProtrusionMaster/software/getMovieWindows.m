...
if usesProt
    ...
    %% Junbong Jang 5/28/2020
    target_pixel_history = protrusion.target_pixel_history;
    protrusion = protrusion.protrusion;
    
    % draw tracked lines
    root_path = movieData.movieDataPath_;
    save_path = '/protrusion_tracking/';
    img_path = '/images/';
    
    % check if the tracked image already exists
    img_struct = dir(fullfile([root_path, img_path], '*.tif'));
    folder_path = img_struct.folder;
    img_names = {img_struct.name};
    final_img_name = img_names{1,end};
    tracked_final_frame_path = [root_path, save_path, strrep(final_img_name, '.tif', '.png')];
    
    % if the tracked image doesn't exist, make it
    if isfile(tracked_final_frame_path)
        tracked_final_frame = imread(tracked_final_frame_path);
    else
        msgbox_track_points_info = msgbox({'Please wait for protrusion','tracking visualization'},'Visualizing Tracked Lines');
        set(msgbox_track_points_info,'Position',[350 499 220 70]);

        tracked_final_frame = drawTrackedLines(target_pixel_history, [root_path, img_path], [root_path, save_path]);
        delete(msgbox_track_points_info);
    end
        
    [clicked_x, clicked_y] = clickImage(tracked_final_frame, 0);
    
    % find which tracked pixel is chosen
    clicked_pixel_a = [clicked_x(1,1), clicked_y(1,1)]
    clicked_pixel_b = [clicked_x(2,1), clicked_y(2,1)]
    
    list_of_tracked_pixels = target_pixel_history(1,:)';
    list_of_tracked_pixels = cell2mat(list_of_tracked_pixels);

    pixel_index_a = findClosestTargetPixel(clicked_pixel_a, list_of_tracked_pixels);
    pixel_index_b = findClosestTargetPixel(clicked_pixel_b, list_of_tracked_pixels);
    
    new_target_pixel_history = target_pixel_history(:,pixel_index_a);
    new_target_pixel_history(:,2) = target_pixel_history(:,pixel_index_b);  % return Nx2 Cell array
    target_pixel_history = new_target_pixel_history;   
end
...
if strcmp(p.MethodName,'ConstantWidth') || strcmp(p.MethodName, 'ConstantNumber') || strcmp(p.MethodName, 'ProtrusionBased')
    ...
    if strcmp(p.MethodName,'ConstantWidth') || strcmp(p.MethodName, 'ConstantNumber')
        ...
        windows = getMaskWindows(maskArray(:,:,1),p.PerpSize,...
            ...
            'StartStripOnly',true,'DoChecks',false, 'iFrame', 1, 'target_pixel_history', target_pixel_history, 'smoothedEdge', smoothedEdge);
        ...
        switch p.MethodName
            ...
            case 'ConstantNumber'
                if iFrame == 1
                    windows_array{iFrame} = getMaskWindows(maskArray(:,:,iFrame),p.PerpSize,...
                         p.ParaSize,'StartPoint',startingPoints{iFrame},'StartContour',p.StartContour,...
                         'DoChecks',false,'iFrame', iFrame, 'target_pixel_history', target_pixel_history, 'smoothedEdge', smoothedEdge);
                else
                    windows_array{iFrame} = getMaskWindows(maskArray(:,:,iFrame),p.PerpSize,...
                        [],'StartPoint',startingPoints{iFrame},'StartContour',p.StartContour,...
                        'NumParallel',nWinPara,'DoChecks',false,'iFrame', iFrame, 'target_pixel_history', target_pixel_history, 'smoothedEdge', smoothedEdge);
                end
